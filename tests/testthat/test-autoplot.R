test_that("autoplot.prcomp works correctly", {
  ...
  p <- autoplot(pca_result, group = iris$Species)
  expect_s3_class(p, "ggplot")

  p <- autoplot(pca_result, group = iris$Species, biplot = TRUE)
  expect_s3_class(p, "ggplot")

  p <- autoplot(pca_result, group = iris$Species) +
    autolayer(pca_result, group = iris$Species)
  expect_s3_class(p, "ggplot")
  
  p <- autoplot(pca_result, group = iris$Species) +
    autolayer(pca_result, group = iris$Species, labels = iris$Species)
  expect_s3_class(p, "ggplot")
