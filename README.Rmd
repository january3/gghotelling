...
```{r example-nocode,echo=FALSE,fig.width=12, fig.height=12}
library(ggplot2)
library(gghotelling)
library(cowplot)
library(ggrepel)
...
p4b <- ggplot(df, aes(PC1, PC2, color=Species)) +
...
p1234 <- plot_grid(p1, p2, p3, p4, p4b, ncol=2)
p5 <- plot_outliers(iris[, 1:2]) +
  ggtitle("Outlier detection")
plot_grid(p1234, p5, ncol=1, rel_heights = c(3, 1))
```
...
See [the vignette](https://january3.github.io/gghotelling/articles/gghotelling.html) for more details.
...
I used LLM modes to a) detect bugs, spelling errors, clerical mistakes and
other deficiencies in the code and b) better understand the ggplot2
